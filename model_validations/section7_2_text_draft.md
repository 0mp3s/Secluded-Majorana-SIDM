...
**Results.** The MAP benchmark ($\alpha = 5.7 \times 10^{-3}$) produces $r_{\rm core} = 829$ pc, enclosing all 5 GCs at face-on and mean deprojections: **14/15 safe** (5 GCs × 3 deprojections). The single marginal case is Fornax 3 at median deprojection ($r_{\rm 3D} = 860$ pc, just outside $r_{\rm core}$), with inspiral time $t_{\rm DF} \sim 0.6$ Gyr. The core size is consistent with the observationally inferred core radius of Fornax ($r_{\rm core} \approx 500$–$900$ pc; Walker \& Peñarrubia 2011; Amorisco \& Evans 2012).
