...
**Vacuum stability.** The quartic self-coupling $\lambda_\phi$ (in the scalar potential $V(\phi) = \frac{1}{2}m_\phi^2\phi^2 + \frac{\mu_3}{3!}\phi^3 + \frac{\lambda_\phi}{4!}\phi^4$) must be positive for the potential to be bounded from below at large field values. The cubic term can generate a secondary local minimum at negative $\phi$; requiring the SIDM-relevant vacuum $\phi = 0$ to be the global minimum imposes $\lambda_\phi > 3\mu_3^2/(8m_\phi^2)$, derived by demanding that $V'(\phi) = 0$ has no real non-trivial solutions (negative discriminant of the resulting quadratic). For the cosmological lower bound $\mu_3/m_\phi = 1.7$, this gives $\lambda_\phi \gtrsim 1.08$ — an $\mathcal{O}(1)$ value natural for a strongly self-interacting dark sector. A numerical scan of $V(\phi_{\rm min})$ confirms the tighter bound $\lambda_\phi \gtrsim 0.96$, below which a secondary minimum deeper than the origin appears. Radiative corrections from the Yukawa interaction contribute $\delta\lambda_\phi \sim y^4/(16\pi^2) \sim \mathcal{O}(10^{-9})$–$\mathcal{O}(10^{-6})$, negligible compared to the tree-level lower bound. Vacuum stability is therefore automatically satisfied within the cannibal regime $\mu_3/m_\phi \gtrsim 1.7$.
