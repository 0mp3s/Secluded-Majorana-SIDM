...
All benchmark-point parameters are loaded from `data/global_config.json` (single source of truth).
Local `config.json` selects benchmarks by label:

```jsonc
{
    "benchmark_labels": ["BP1", "MAP", "MAP_relic"],
    "benchmark_label": "MAP",
    "higgs_portal": {
